...
import { ag as Sa, k as ks, G as Ge, O, m as me, v as ve, u as ue, n as ne, c as ce, a9 as Ue, aa as Se, ab as Fe, j as v, h as la, E as Ee$1, _ as _e, q as Oe, T as Te, a3 as ge, a1 as te, e as ee, a2 as Nt, ah as Ts, a0 as dt } from '../nitro/nitro.mjs';
...
import { N as Ne, p as pe, x as xe, y as ye, l as le } from './collapsible-BpTvkUeS.mjs';
import { toast } from 'sonner';
import { b, f, p, u, y, g, h, v as v$1 } from './dialog-Bz9OJgDB.mjs';
import { d } from './checkbox-C7Fdz6Lt.mjs';
...
function it({ project: l, onProjectUpdated: g$1, trigger: u$1, userId: v }) {
  const A = useNavigate(), P = useSetAtom(ks), [b$1, C] = useState(false), [j, M] = useState(false), [k, D] = useState(false), [E, N] = useState(false), [i, w] = useState(""), [m, F] = useState({ name: "", description: "", isPrivate: false }), [S, T] = useState(true);
  useEffect(() => {
    var _a;
    if (b$1) {
...
const je = ({ teamspaceId: l = null, projectId: g = null, chatroomId: u$1 = null, organizationId: v$1, receiverIds: A = [], onUploadComplete: P, compact: b$1 = false, files: C = [], className: j = "" }) => {
  ...
  var _a, _b, _c, _d, _e2;
  const { projectId: g$1 } = Sa.useParams(), [u$1, v$1] = useState(false), [A, P] = useState(true), [b$1, C] = useState(""), [j, M] = useState(""), [k, D] = useState(false), [E, N] = useState(false), [i, w] = useState(null), [m, F] = useState(false), [S, T] = useAtom(ks), R = useSetAtom(Ge), $ = Sa.useRouteContext(), [O$1] = useState($.userId), s = S.flatMap((a) => a.projects || []).find((a) => a.id === g$1), r = S.find((a) => a.id === (s == null ? void 0 : s.teamspaceId));
  useEffect(() => {
  ...
  const f$1 = async (a = "") => {
    ...
    if (u$1) return;
    v$1(true);
    const c = Ts({ currentUserId: O$1, project: s, teamspace: r });
    try {
    ...
    if (!(!b$1.trim() || u$1)) try {
      v$1(true);
      const a = Ts({ currentUserId: O$1, project: s, teamspace: r }), { data: c } = await ge().chatrooms.createChatroom({ chatroomCreatorId: O$1, chatroomMembers: a, isPrivate: false, projectId: s.id, name: "New Chat Conversation", teamIds: [], type: "PROJECT_CHATROOM" });
...
//# sourceMappingURL=_projectId-sGg0phwN.mjs.map
